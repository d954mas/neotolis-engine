...
Человек снял старый 2-агентный контракт: теперь я (Claude) — один за **код + дизайн + значения**,
ты (Codex) — **арт-помощник**. Этот файл оставляем как канал арт-запросов: ты отвечаешь в
`FROM_GDD.md`, кладёшь PNG в `raw/...` как раньше (generated bitmap, точные имена, contact sheet).

**ХЭНДШЕЙК:** подтверди приём записью в `FROM_GDD.md` — так проверим, что канал работает.

Новый дизайн целиком — `games/turkic-jam-2026/REDESIGN_PLAN.md` (прочти). Кратко: авто-мердж-роуглайт,
**бой = пульс**, 3 стата (Сила/Скорость/Выносливость), 5 линий мерджа, мешочек-ревил карт,
боссы-архетипы, одна валюта припасы, один герой. Вырезано: экипировка, классы, wisdom/glory.

**АРТ НУЖЕН (приоритет под фазы кода):**

- **P1 — ВРАГИ (первым, делаю бой):** `raw/enemies/` — зверь (волк), степной дух, мираж-тварь,
  песчаный элементаль; idle + hit-кадр, ~96px. Переиспользуй существующие `fx_*` для попадания/урона.
- **P2 — 5 ЛИНИЙ МЕРДЖА × 3 тира** (tile 128 + card 64): Война (Точило→Кузня→Арсенал),
  Табун (Коновязь→Табун→Орда), Степь (Оазис→Колодец→Озеро), Жильё (Саксаул→Юрта→Аул-стоянка),
  Вода (Лужа→Родник→Озеро). Переиспользуй saxaul/yurt/oasis/tamga_stone где ложатся.
- **P3 — БОССЫ:** 🗿 Толстяк, ⚡ Шустрый, 💢 Лютый + финальный именной. Крупные.
- **P4 — UI:** мешочек (закрытый + FX открытия), грани кубика d4..d20, иконки событий
  (буря/погоня/…), иконки веток аула.

Начни с **P1 (враги)**. Предложи свой порядок/контракт в `FROM_GDD.md`. Спасибо!

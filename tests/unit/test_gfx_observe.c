...
static void test_render_frames_sum_and_begin_tick_resets(void) {
...
/* begin_tick wipes a new loss; frame and pass calls on the lost context are no-ops, not traps. */
static void test_loss_is_wiped_at_begin_tick_and_frame_calls_are_no_ops(void) {
    nt_program_t program = nt_gfx_fake_make_program(NULL, 0);
...
/* A loss inside an iteration changes no state: its work is issued and does nothing, and the next begin_tick wipes. */
static void test_loss_during_an_iteration_is_wiped_at_the_next_begin_tick(void) {
    nt_program_t program = nt_gfx_fake_make_program(NULL, 0);
    nt_gfx_begin_frame();
    nt_gfx_fake_set_context_lost(true);
    nt_gfx_begin_pass(&(nt_pass_desc_t){.clear_depth = 1.0F});
...
/* A loss and restore between two iterations (a background tab) wipe and restore in one begin_tick. */
static void test_loss_and_restore_between_iterations_restore_in_one_begin_tick(void) {
    nt_program_t program = nt_gfx_fake_make_program(NULL, 0);
...
static void test_begin_tick_with_an_open_render_frame_asserts(void) {
    nt_gfx_begin_frame();
...
/* The begin_tick that consumes a request starts recording the tick it opens. */
...
/* The wipe runs before the tick opens and the restore inside it, so the snapshot
...
static void test_capture_prefix_lifetime_and_saved_snapshot(void) {
    ...
    /* A request leaves the finished capture intact until its begin_tick starts recording. */
    nt_gfx_capture_request();
    nt_gfx_begin_frame();
    nt_gfx_end_frame();
...
/* A request during a recorded tick replaces that capture at the begin_tick that finishes it. */
...
int main(void) {
    ...
    UNITY_BEGIN();
    RUN_TEST(test_render_frames_sum_and_begin_tick_resets);
    RUN_TEST(test_instanced_products_are_widened_before_multiplication);
    RUN_TEST(test_loss_is_wiped_at_begin_tick_and_frame_calls_are_no_ops);
    RUN_TEST(test_loss_during_an_iteration_is_wiped_at_the_next_begin_tick);
    RUN_TEST(test_creates_on_a_loss_fail_quietly);
    RUN_TEST(test_loss_and_restore_between_iterations_restore_in_one_begin_tick);
    RUN_TEST(test_first_tick_counts_initial_resource_creation);
    RUN_TEST(test_shutdown_discards_an_open_tick);
#if NT_ASSERT_MODE == NT_ASSERT_FULL
    RUN_TEST(test_begin_tick_with_an_open_render_frame_asserts);
#endif
#if NT_GFX_CAPTURE_ENABLED

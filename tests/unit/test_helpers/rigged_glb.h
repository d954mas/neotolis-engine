 ...
 *                  (with root_translation the matrix also translates by
 *                  (1, -2, 2), so the rig root's rest is 3 from the origin)
 ...
 *                    key1 in (-8, -8, -8) value (1, 2, 4) out (100, 100, 100)
 *                    The unused tangents are loud so a swapped tangent order
 *                    shows; at t = 0.5 the spline is exactly (1.5, 1.5, 2.5),
 *                    and the in-tangent pulls the curve past its end value
 *                    between the keys.
...
typedef struct {
    ...
    bool root_translation;           /* Root's matrix gains the translation (1, -2, 2): a rig root off the origin, not a defect */
    ...
    bool animation_negated_last_key; /* the Joint1 LINEAR rotation is identity at 0, 0.25 and 0.5 s and (0, 0, 0, -1) held from 1.0 s: one rotation, two signs */

...
_Static_assert(offsetof(nt_atlas_opts_t, tracer_tolerance) > offsetof(nt_atlas_opts_t, gen_mipmaps), "atlas controls must be append-only");
_Static_assert(offsetof(nt_atlas_sprite_opts_t, tracer_tolerance) > offsetof(nt_atlas_sprite_opts_t, extrude), "sprite controls must be append-only");
...
int main(void) {
    ...
    if (atlas.compress != &compress || atlas.format != NT_TEXTURE_FORMAT_RG8 || atlas.max_size != 101 || atlas.padding != 102 || atlas.margin != 103 || atlas.extrude != 104 ||
        ...
        atlas.wrap_u != NT_TEXTURE_DEFAULT_WRAP_CLAMP_TO_EDGE || atlas.wrap_v != NT_TEXTURE_DEFAULT_WRAP_CLAMP_TO_EDGE || atlas.gen_mipmaps || atlas.tracer_tolerance != 0.0F) {
        return 1;
    }
    if (sprite.name == NULL || sprite.name[0] != 'l' || sprite.origin_x != 0.25F || sprite.origin_y != 0.75F || sprite.slice9_left != 11 || sprite.slice9_right != 12 || sprite.slice9_top != 13 ||
        ...
        sprite.extrude != 16 || sprite.tracer_tolerance != 0.0F || sprite.alpha_threshold != 0) {
        return 2;
    }
    return 0;
}

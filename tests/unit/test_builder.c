...
void test_hash_known_value(void) {
    uint64_t h = nt_builder_normalize_and_hash("test");
    TEST_ASSERT_TRUE(h != 0);
    /* Deterministic */
    TEST_ASSERT_TRUE(h == nt_builder_normalize_and_hash("test"));
}

void test_hash_path_normalization(void) {
    /* Backslash -> forward slash */
    uint64_t h1 = nt_builder_normalize_and_hash("meshes/cube.glb");
    uint64_t h2 = nt_builder_normalize_and_hash("meshes\\cube.glb");
    TEST_ASSERT_TRUE(h1 == h2);

    /* ./ stripped */
    TEST_ASSERT_TRUE(nt_builder_normalize_and_hash("foo/bar.png") == nt_builder_normalize_and_hash("./foo/bar.png"));

    /* // collapsed */
    TEST_ASSERT_TRUE(nt_builder_normalize_and_hash("foo/bar.png") == nt_builder_normalize_and_hash("foo//bar.png"));

    /* ../ resolved */
    TEST_ASSERT_TRUE(nt_builder_normalize_and_hash("bar.png") == nt_builder_normalize_and_hash("foo/../bar.png"));

    /* Leading ../ preserved */
    TEST_ASSERT_TRUE(nt_builder_normalize_and_hash("assets/mesh.glb") != nt_builder_normalize_and_hash("../assets/mesh.glb"));
}

void test_hash_different_strings_differ(void) {
    uint64_t h1 = nt_builder_normalize_and_hash("a");
    uint64_t h2 = nt_builder_normalize_and_hash("b");
    TEST_ASSERT_TRUE(h1 != h2);
}
...
void test_rename_changes_resource_id(void) {
    ...
    uint64_t old_id = nt_builder_normalize_and_hash(vert_path);
    uint64_t new_id = nt_builder_normalize_and_hash("renamed/shader.vert");
    ...
    NtAssetEntry entry;
    (void)fread(&entry, sizeof(entry), 1, f);
    TEST_ASSERT_TRUE(new_id == entry.resource_id);
    (void)fclose(f);
}

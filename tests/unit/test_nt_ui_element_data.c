...
static void test_layer_only_macro(void) {
    const nt_ui_element_data_t *d = NT_UI_DATA_LAYER(5);
...
static void test_full_macro(void) {
    int payload = 42;
    const nt_ui_element_data_t *d = NT_UI_DATA_FULL(7, &payload);
    TEST_ASSERT_NOT_NULL(d);
...
static void test_layer_only_no_scratch(void) {
    const size_t before = nt_mem_scratch_used();
    const nt_ui_element_data_t *a = NT_UI_DATA_LAYER(0);
    const nt_ui_element_data_t *b = NT_UI_DATA_LAYER(1);
    (void)a;
    (void)b;
    TEST_ASSERT_EQUAL_UINT64((uint64_t)before, (uint64_t)nt_mem_scratch_used());
}

static void test_full_with_user_data_uses_scratch(void) {
    int payload = 1;
    const size_t before = nt_mem_scratch_used();
    const nt_ui_element_data_t *d = NT_UI_DATA_FULL(2, &payload);
    (void)d;
    TEST_ASSERT_TRUE(nt_mem_scratch_used() > before);
}
...
static void test_layer_only_same_layer_same_ptr(void) {
    const nt_ui_element_data_t *a = NT_UI_DATA_LAYER(3);
    const nt_ui_element_data_t *b = NT_UI_DATA_LAYER(3);
    const nt_ui_element_data_t *c = NT_UI_DATA_LAYER(4);
...
static void test_full_distinct_allocations(void) {
    int p1 = 1;
    int p2 = 2;
    const nt_ui_element_data_t *a = NT_UI_DATA_FULL(0, &p1);
    const nt_ui_element_data_t *b = NT_UI_DATA_FULL(0, &p2);
...
static void test_reset_releases_macro_storage(void) {
    int payload = 0;
    const nt_ui_element_data_t *tmp = NT_UI_DATA_FULL(1, &payload);
    (void)tmp;
    const size_t after_alloc = nt_mem_scratch_used();
    TEST_ASSERT_TRUE(after_alloc > 0U);
    nt_mem_scratch_reset();
    TEST_ASSERT_EQUAL_UINT64(0U, (uint64_t)nt_mem_scratch_used());
    const nt_ui_element_data_t *d = NT_UI_DATA_LAYER(2);
    TEST_ASSERT_EQUAL_UINT8(2U, d->layer);
}

/* T9: layer=255 boundary value. */
static void test_layer_boundary_255(void) {
    const nt_ui_element_data_t *d = NT_UI_DATA_LAYER(255);
    TEST_ASSERT_NOT_NULL(d);
...
static void test_full_with_null_user_data_uses_table(void) {
    const nt_ui_element_data_t *via_layer = NT_UI_DATA_LAYER(10);
    const nt_ui_element_data_t *via_full = NT_UI_DATA_FULL(10, NULL);

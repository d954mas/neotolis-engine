...
void setUp(void) {
    skeletal_rig_asymmetric(&g_rig);
    skeletal_rig_bindings(&g_rig, &g_a, &g_b);
}

void tearDown(void) {}

/* Bind pose with arm and tail1 rotated: a pose no inverse bind cancels. */
static void make_edited_pose(nt_skeletal_trs_t *local) {
    memcpy(local, g_rig.bind, (size_t)SKELETAL_RIG_JOINT_COUNT * sizeof(nt_skeletal_trs_t));
    skeletal_rig_set_axis_angle(&local[JOINT_ARM], 0.0F, 0.0F, 1.0F, 47.0F);
    skeletal_rig_set_axis_angle(&local[JOINT_TAIL1], 0.0F, 1.0F, 0.0F, -80.0F);
}

static bool is_identity(const nt_skeletal_mat34_t *m, float tol) {
    for (int r = 0; r < 3; ++r) {
        for (int c = 0; c < 4; ++c) {
...
void test_palette_at_bind_pose_is_identity(void) {
    ...
    nt_skeletal_mat34_t palette_a[SKELETAL_RIG_PALETTE_A_COUNT];
    nt_skin_palette_build(&g_a, model, SKELETAL_RIG_JOINT_COUNT, palette_a, SKELETAL_RIG_PALETTE_A_COUNT);
    for (uint16_t p = 0; p < g_a.palette_count; ++p) {
        TEST_ASSERT_TRUE(is_identity(&palette_a[p], 1e-5F));
    }

    nt_skeletal_mat34_t palette_b[SKELETAL_RIG_PALETTE_B_COUNT];
    nt_skin_palette_build(&g_b, model, SKELETAL_RIG_JOINT_COUNT, palette_b, SKELETAL_RIG_PALETTE_B_COUNT);
...
void test_shared_joint_matches_in_both_bindings(void) {
    ...
    TEST_ASSERT_EQUAL_MEMORY(&palette_a[HAND_ENTRY_A], &palette_b[HAND_ENTRY_B], sizeof(nt_skeletal_mat34_t));

    /* Equal to each other proves nothing unless both are also the right matrix. */
    mat4 ref_pose[SKELETAL_RIG_JOINT_COUNT];
    skeletal_rig_ref_fk(local, ref_pose);
    mat4 ref_bind[SKELETAL_RIG_JOINT_COUNT];
    skeletal_rig_ref_fk(g_rig.bind, ref_bind);
    ...
    skeletal_rig_assert_mat34_equals_mat4(&palette_a[HAND_ENTRY_A], expected, 1e-5F);
    skeletal_rig_assert_mat34_equals_mat4(&palette_b[HAND_ENTRY_B], expected, 1e-5F);
}
...
void test_empty_palette_writes_nothing(void) {
    ...
    nt_skeletal_mat34_t out[1];
    memset(out, 0x5A, sizeof(out));
    nt_skeletal_mat34_t poison[1];
    memset(poison, 0x5A, sizeof(poison));

    nt_skin_binding_t empty = g_a;
    empty.palette_count = 0;
    nt_skin_palette_build(&empty, model, SKELETAL_RIG_JOINT_COUNT, out, 0);
...
void test_palette_matches_cglm_reference(void) {
    ...
    for (size_t k = 0; k < sizeof(bindings) / sizeof(bindings[0]); ++k) {
        ...
        nt_skeletal_mat34_t palette[SKELETAL_RIG_PALETTE_A_COUNT];
        nt_skin_palette_build(binding, model, SKELETAL_RIG_JOINT_COUNT, palette, SKELETAL_RIG_PALETTE_A_COUNT);
        ...
        for (uint16_t p = 0; p < binding->palette_count; ++p) {
            ...
            skeletal_rig_assert_mat34_equals_mat4(&palette[p], expected, 1e-5F);
        }
    }
}
...
void test_palette_build_traps_on_small_capacity(void) {
    ...
    NT_TEST_EXPECT_ASSERT(nt_skin_palette_build(&g_a, model, SKELETAL_RIG_JOINT_COUNT, palette, SKELETAL_RIG_PALETTE_A_COUNT - 1));
}
...
void test_palette_build_traps_on_out_of_range_remap(void) {
    ...
    uint16_t remap[SKELETAL_RIG_PALETTE_A_COUNT];
    memcpy(remap, g_a.remap, sizeof(remap));
    remap[2] = SKELETAL_RIG_JOINT_COUNT;

    nt_skin_binding_t broken = g_a;
    broken.remap = remap;
    NT_TEST_EXPECT_ASSERT(nt_skin_palette_build(&broken, model, SKELETAL_RIG_JOINT_COUNT, palette, SKELETAL_RIG_PALETTE_A_COUNT));
}
#endif

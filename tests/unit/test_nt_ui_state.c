...
/* Single test tag — these unit tests exercise the pool mechanics, not cross-widget aliasing. */
#define T_TAG NT_UI_STATE_TAG('t', 's', 't', '0')
...
static void test_state_create_returns_zeroed(void) {
    ...
    uint8_t *p = (uint8_t *)nt_ui_state(s_fx.ctx, id, 16U, T_TAG);
...
static void test_state_reacquire_same_pointer(void) {
    ...
    scroll_state_t *a = (scroll_state_t *)nt_ui_state(s_fx.ctx, id, sizeof(scroll_state_t), T_TAG);
    TEST_ASSERT_NOT_NULL(a);
    a->offset_x = 42.0F;
    a->velocity_y = -7.0F;

    scroll_state_t *b = (scroll_state_t *)nt_ui_state(s_fx.ctx, id, sizeof(scroll_state_t), T_TAG);
...
static void test_state_find_absent_null(void) {
    ...
    (void)nt_ui_state(s_fx.ctx, 0x3333U, 8U, T_TAG);
...
static void test_state_clear_then_recreate_zeroed(void) {
    ...
    uint32_t *p = (uint32_t *)nt_ui_state(s_fx.ctx, id, sizeof(uint32_t), T_TAG);
    *p = 0xDEADBEEFU;
    TEST_ASSERT_EQUAL_UINT32(1U, nt_ui_state_used_slots(s_fx.ctx));
    ...
    uint32_t *q = (uint32_t *)nt_ui_state(s_fx.ctx, id, sizeof(uint32_t), T_TAG);
...
static void test_state_clear_all_empties(void) {
    (void)nt_ui_state(s_fx.ctx, 0x10U, 8U, T_TAG);
    (void)nt_ui_state(s_fx.ctx, 0x20U, 16U, T_TAG);
    (void)nt_ui_state(s_fx.ctx, 0x30U, 4U, T_TAG);
...
static void test_state_open_address_coexists(void) {
    ...
    uint32_t *a = (uint32_t *)nt_ui_state(s_fx.ctx, id1, sizeof(uint32_t), T_TAG);
    uint32_t *b = (uint32_t *)nt_ui_state(s_fx.ctx, id2, sizeof(uint32_t), T_TAG);
    TEST_ASSERT_TRUE_MESSAGE(a != b, "linear probe must land id2 in a different slot than id1");
    *a = 1U;
    *b = 2U;
    TEST_ASSERT_EQUAL_UINT32(1U, *(uint32_t *)nt_ui_state(s_fx.ctx, id1, sizeof(uint32_t), T_TAG));
    TEST_ASSERT_EQUAL_UINT32(2U, *(uint32_t *)nt_ui_state(s_fx.ctx, id2, sizeof(uint32_t), T_TAG));
}

/* ---- Test 7: size == 0 is legal (cell with no payload still occupies a slot). ---- */
static void test_state_zero_size_payload(void) {
    void *p = nt_ui_state(s_fx.ctx, 0x7777U, 0U, T_TAG);
    TEST_ASSERT_NOT_NULL(p);
    TEST_ASSERT_EQUAL_UINT32(1U, nt_ui_state_used_slots(s_fx.ctx));
    TEST_ASSERT_EQUAL_UINT32(0U, nt_ui_state_used_bytes(s_fx.ctx));
}
...
static void test_assert_size_mismatch(void) {
    ...
    NT_TEST_EXPECT_ASSERT((void)nt_ui_state(s_fx.ctx, id, 32U, T_TAG));
}

/* Two different widgets colliding on one id+size -> assert (cross-widget aliasing trap). */
static void test_assert_tag_mismatch(void) {
    const uint32_t id = 0xACE0U;
    (void)nt_ui_state(s_fx.ctx, id, 8U, NT_UI_STATE_TAG('s', 'c', 'r', 'l'));
    NT_TEST_EXPECT_ASSERT((void)nt_ui_state(s_fx.ctx, id, 8U, NT_UI_STATE_TAG('s', 'l', 'd', 'g')));
}

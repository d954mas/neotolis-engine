...
static void test_initial_uniform_records_cover_only_vec4(void) {
    ...
    nt_gfx_capture_request();
    nt_gfx_end_tick();
    nt_gfx_end_tick(); /* a recorded tick without gfx work still snapshots inherited state */
    nt_gfx_capture_view_t capture = nt_gfx_capture_read();
    uint32_t records = 0;
    for (uint32_t i = 0; i < capture.count; i++) {
        const nt_gfx_event_t *event = &capture.events[i];
...
#endif
/* Uploads before begin_frame land in the open tick. */
static void test_payloads_before_render_land_in_their_tick(void) {
    ...
    nt_gfx_end_tick();
    TEST_ASSERT_EQUAL_UINT64(16, g_nt_gfx.last_tick.counters.buffer_upload_bytes);
    ...
    nt_gfx_end_frame();
    nt_gfx_end_tick();
    const nt_gfx_tick_snapshot_t *end = &g_nt_gfx.last_tick;
...
static void test_r8_odd_width_update_counts_exact_bytes(void) {
    ...
    nt_gfx_end_tick();
    TEST_ASSERT_EQUAL_UINT64(1, g_nt_gfx.last_tick.counters.texture_upload_calls);
    TEST_ASSERT_EQUAL_UINT64(3, g_nt_gfx.last_tick.counters.texture_upload_bytes);
}
...
static void test_failed_upload_keeps_issued_bytes(void) {
    ...
    for (uint32_t i = 0; i < 2; i++) {
        ...
        TEST_ASSERT_EQUAL(NT_GFX_TICK_COMPLETE, snapshot->status);
    }
}
...
static void test_repeated_frames_separate_requests_from_issued_calls(void) {
    ...
    for (uint32_t frame = 0; frame < 2; frame++) {
...
#endif
        nt_gfx_counters_t c = g_nt_gfx.last_tick.counters;
        ...
        /* An empty tick starts the next recording only after this one was read. */
        nt_gfx_capture_request();
        nt_gfx_end_tick();
#endif
    }
...
static void test_compressed_mips_use_issued_block_sizes(void) {
    ...
    nt_gfx_end_tick();
    nt_gfx_counters_t counters = g_nt_gfx.last_tick.counters;

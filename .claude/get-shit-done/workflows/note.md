...
- **Global scope**: `C:/projects/neotolis-engine/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/projects/neotolis-engine/.claude/notes/`)
...
2. Glob `C:/projects/neotolis-engine/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/projects/neotolis-engine/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/projects/neotolis-engine/.claude/notes/` — works in any directory

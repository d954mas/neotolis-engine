...
Plans:
- [ ] 23-01-PLAN.md -- Vendor cgltf + stb_image, nt_builder static library, public API, pack writer core, FNV-1a hash, pack_dump
- [ ] 23-02-PLAN.md -- Asset importers: mesh (cgltf + type conversion), texture (stb_image RGBA8), shader (comment strip + validation)
- [ ] 23-03-PLAN.md -- Glob patterns, unit tests (15+), test fixtures, demo builder example
...
Plans:
- [ ] 24-01-PLAN.md -- Module skeleton: nt_resource types, NEOPAK parser, handle pool, FNV-1a hash, CMake, 15+ unit tests
- [ ] 24-02-PLAN.md -- Pack management: mount/unmount/set_priority, request/get/is_ready/get_state, step() resolve, 16+ unit tests
- [ ] 24-03-PLAN.md -- Virtual packs: create_pack/register/unregister, placeholder fallback, ownership semantics, 13+ unit tests
...
**Success Criteria** (what must be TRUE):
  1. Pack loading on WASM uses JS fetch bridge (nt_http with EM_JS fetch + ReadableStream) with state machine NONE -> REQUESTED -> DOWNLOADING -> LOADED -> READY / FAILED
  2. Pack loading on native uses synchronous fread() via nt_fs with the same state transitions
  3. resource_step() processes loaded packs each frame: activator callbacks create GPU resources (shader compile, texture upload, mesh VBO+IBO)
  4. Shader asset activation compiles shader via nt_gfx_make_shader (pipeline deferred to Phase 26/27)
  5. Pack blob persists in memory with configurable per-pack blob policy (KEEP or AUTO-evict after TTL)
**Plans**: 3 plans

Plans:
- [ ] 25-01-PLAN.md -- nt_http + nt_fs I/O modules: web/native/stub backends, generational handles, take semantics, CMake, unit tests
- [ ] 25-02-PLAN.md -- nt_gfx activators (texture/mesh/shader), mesh side table, context loss refactor, nt_shape_restore_gpu
- [ ] 25-03-PLAN.md -- nt_resource pack loading state machine, I/O integration, activator callbacks, budget, retry, blob eviction, invalidation
...
Plans:
- [ ] 26-01-PLAN.md -- Rename nt_render_state_comp to nt_drawable_comp, extend nt_gfx_mesh_info_t with stream descriptors
- [ ] 26-02-PLAN.md -- nt_material module: pooled handles, descriptor creation, resource resolution step, query API, 16+ unit tests
...
Plans:
- [ ] 27-01-PLAN.md -- Render defs (globals, instance, item structs), sort key helpers, visibility utility, UBO buffer support, drawable tag migration to nt_hash32_t
- [ ] 27-02-PLAN.md -- Mesh renderer module: pipeline cache, run detection, instanced drawing, draw_list API
...
Plans:
- [x] 28-01-PLAN.md -- Builder extensions: nt_glb_scene API, NT_ASSET_BLOB, tangent support, texture-from-memory
- [ ] 28-02-PLAN.md -- Engine global block registration + blob resource access
- [ ] 28-03-PLAN.md -- Sponza assets, shaders, build_packs.c producing two progressive packs
- [ ] 28-04-PLAN.md -- Runtime demo: entity setup from manifest, Blinn-Phong rendering, FPS camera
- [ ] 28-05-PLAN.md -- CI integration, WASM build, visual verification
...
| 25. Asset Loading | 3/3 | Complete    | 2026-03-18 | - |
| 26. Material System | 2/2 | Complete    | 2026-03-19 | - |
| 27. Mesh Rendering Pipeline | 2/2 | Complete    | 2026-03-19 | - |
| 28. Demo Integration | v1.3 | 0/5 | Planning | - |
| 29. nt_hash module | 3/3 | Complete    | 2026-03-18 | - |

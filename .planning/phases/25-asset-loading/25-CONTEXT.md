...
- **nt_fs web backend via Emscripten IDBFS** — persistent storage on web via IndexedDB for save games
- **nt_http native backend via libcurl** — HTTP requests from desktop (download packs from server)
- **Time-based activation budget** — adaptive budget based on frame time instead of fixed cost. Requires timer query infrastructure.
- **SHDR-02 pipeline creation at activation** — deferred to Phase 26/27 where vertex layout and render state are known
- **Blob memory pool** — preallocated memory for pack blobs to avoid malloc fragmentation
- **nt_gfx full context loss migration** — when all modules use nt_resource, restore_context() can be fully removed. Shape renderer and other direct-gfx users need gradual migration.

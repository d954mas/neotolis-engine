...
- [ ] **BUILD-04**: Builder writes NEOPAK binary pack with header, asset entries, and aligned data
- [ ] **BUILD-05**: Builder supports code-first API (start_pack/add_mesh/add_texture/add_shader/finish_pack)
...
- [ ] **REG-01**: Asset registry tracks resources by ResourceId with AssetMeta (type, pack_index, offset, size, state)
- [ ] **REG-02**: Asset state machine transitions: REGISTERED -> LOADING -> READY / FAILED
- [ ] **REG-03**: Pack stacking with priority -- higher pack_index overrides lower for same ResourceId
- [ ] **REG-04**: Game can mount/unmount packs and change priority order at runtime
- [ ] **REG-05**: Runtime-created resources can be registered in the registry by name (two-level system)
- [ ] **REG-06**: Typed handles: MeshHandle, TextureHandle, ShaderHandle for type-safe access

### Asset Loading

- [ ] **LOAD-01**: Pack loading on WASM via JS fetch bridge (platform_request_fetch / on_fetch_complete)
- [ ] **LOAD-02**: Pack loading on native via synchronous fread()
- [ ] **LOAD-03**: Pack state machine: NONE -> REQUESTED -> LOADED -> READY / FAILED
- [ ] **LOAD-04**: NEOPAK parser validates magic, version, sizes, CRC32 before registering assets
...
- [ ] **TEX-01**: nt_gfx_make_texture() creates GPU texture from raw RGBA8 data with configurable filter/wrap
- [ ] **TEX-02**: nt_gfx_bind_texture() binds texture to sampler slot for rendering
- [ ] **TEX-03**: nt_gfx_destroy_texture() releases GPU texture
- [ ] **TEX-04**: Texture pool follows existing slot/generation pattern (like shaders, buffers, pipelines)
- [ ] **TEX-05**: glGenerateMipmap() support when requested in texture descriptor
...
- [ ] **ENT-01**: EntityHandle with uint16 index + uint16 generation for stale handle detection
- [ ] **ENT-02**: Fixed entity pool with compile-time MAX_ENTITIES limit
- [ ] **ENT-03**: Entity create/destroy/is_alive API with generation validation
- [ ] **ENT-04**: Per-entity alive and enabled flags
...
| TEX-01 | Phase 21 | Pending |
| TEX-02 | Phase 21 | Pending |
| TEX-03 | Phase 21 | Pending |
| TEX-04 | Phase 21 | Pending |
| TEX-05 | Phase 21 | Pending |
...
| REG-04 | Phase 24 | Pending |
| REG-05 | Phase 24 | Pending |
| REG-06 | Phase 24 | Pending |

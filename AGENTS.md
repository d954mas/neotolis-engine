...
- **CI debug ctest runs under ASan/LeakSanitizer** — an `NT_TEST_EXPECT_ASSERT` that longjmps past a live `malloc` fails the test with a leak report printed *after* Unity's `OK`; place link/validation asserts after the `free`, not inside the loop that owns the buffer.

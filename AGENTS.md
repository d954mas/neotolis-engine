...
Neotolis Engine — минималистичный игровой движок на **C23** для **Web/WASM** (WebGL 2). Архитектура data-oriented, code-first. Builder делает тяжёлую работу офлайн, runtime намеренно простой.
...
Если соответствующая инфраструктура уже есть в репозитории, после изменений по возможности запускать:

- сборку затронутых target'ов;
- релевантные тесты и smoke checks;
- минимальную проверку, что изменение не ломает code-first orchestration и platform abstraction.

Если команды сборки или тестов отсутствуют, это нужно прямо указать в итоговом ответе, а не подразумевать проверку.

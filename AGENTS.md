...
- **Fail early, prefer asserts** — when something is wrong, prefer crashing over silent fallbacks. `NT_ASSERT` for invariants and unexpected states, `NT_BUILD_ASSERT` in the builder (programmer invariants, unexpected states, OOM, a missing/unreadable file, and a decode failure in the single-asset adds). Error returns are fine as part of public API contracts (resource not found → NULL), but never to silently swallow a bug or broken data. Release default is TRAP; OFF is available via CMake for final production builds.
  - `NT_ASSERT` compiles to `((void)0)` under `NT_ASSERT_MODE=OFF` (a legal shipping config): never put a side effect or the only bounds check inside an assert — parsers of untrusted data need hard guards that survive assert-off.

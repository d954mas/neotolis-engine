...
- Real Basis coverage uses `bash scripts/check_basisu_browser.sh` after activating emsdk and installing `tests/browser` npm/Chromium dependencies. It encodes a fresh fixture pack and rebuilds the real-transcoder WASM Debug/Release fixtures, then checks all five CPU outputs and RGB/alpha/mip sampling through the default GPU format. `NT_BASIS_HARDWARE=1 NT_BASIS_PORT=8463 bash scripts/check_basisu_browser.sh` requires a hardware renderer; other GPU formats remain unverified.

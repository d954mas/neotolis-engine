...
- **Fail early, prefer asserts** — when something is wrong, prefer crashing over silent fallbacks. `NT_ASSERT` for invariants and unexpected states, `NT_BUILD_ASSERT` in the builder. Error returns are fine as part of public API contracts (e.g. resource not found → return NULL), but don't use error codes to silently swallow problems that indicate bugs or broken data. Release default is TRAP (immediate crash, no strings); OFF mode is available via CMake override for final production builds. In the builder, programmer invariants, unexpected states, and OOM assert (`NT_BUILD_ASSERT`) — the developer sees the problem instantly instead of waiting for the build to finish. A missing or unreadable file also asserts. But CONTENT-dependent failures of present, well-formed inputs — undecodable/oversized/zero-dimension images, transparent-after-trim sprites, degenerate hulls, duplicate names, size/page limits, unfittable sprites — route to the graceful content-error channel (`nt_builder_get_errors`) instead of aborting, so the builder and any GUI survive one bad asset with an actionable message.

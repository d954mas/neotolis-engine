...
- Asset activators validate structure (what a view needs to address memory), never values inside a sound payload; values are builder asserts, the pack CRC32 and the kernels' check flags. A value check in an activator is a spec change, not hardening (see the skeletal chapter's locked rule).

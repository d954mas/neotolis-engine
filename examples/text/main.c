...
#define ZOOM_SPEED 0.32F /* per wheel notch (wheel_dy is now notch-normalized: 1.0/detent) */
...
static void frame(void) {
     ...
     * from window edge being interpreted as camera rotation */
    bool resized = (g_nt_window.fb_width != s_prev_fb_w || g_nt_window.fb_height != s_prev_fb_h);
    if (!resized) {
        trackball_update(dt);
    }

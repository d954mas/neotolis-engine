 ...
 * Renders a single row of reference buttons (text/icon/icon+text/disabled,
 * eased vs instant) so the per-state container visual + transitions can be
 * eye-checked. Click counter proves WIDGET-02 click-on-release fires once
 * per click; ui_pointer flag proves nt_ui_wants_pointer gates world input.
 * No panel/transform/slice9-stretch demos -- those live in slice9_demo.
 * Keys: Esc quit.
...
static void declare_reference_buttons(void) {
    ...
    CLAY({.id = CLAY_ID("ref-btn-title"), .layout = {.sizing = {CLAY_SIZING_GROW(0), CLAY_SIZING_FIT(0)}, .childAlignment = {CLAY_ALIGN_X_CENTER, CLAY_ALIGN_Y_CENTER}}}) {
        nt_ui_label(s_ctx, NT_UI_DATA_LAYER(LAYER_TEXT), "Reference buttons: hover / press; eased vs instant; disabled; icon+text", &g_title_style);
    }
...
static void frame(void) {
    ...
    if (can_render) {
        ...
        (void)snprintf(status_text, sizeof status_text, "clicks: %u    ui_pointer: %s    [Esc] quit", s_click_count, nt_ui_wants_pointer(s_ctx) ? "yes" : "no");
        // #endregion
...
int main(int argc, char *argv[]) {
    ...
    nt_log_info("ui_buttons_demo: starting (hover/press the reference buttons; eased vs instant)");

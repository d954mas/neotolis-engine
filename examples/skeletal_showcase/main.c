...
static void make_camera_vp(mat4 vp, float aspect, float eye[3]) {
    ...
    eye[0] = sinf(s_camera_yaw) * cp * s_camera_distance;
    eye[1] = 1.55F + sinf(s_camera_pitch) * s_camera_distance;
    eye[2] = cosf(s_camera_yaw) * cp * s_camera_distance;
...
static void declare_joint_list(void) {
    CLAY({.layout = {.sizing = {CLAY_SIZING_FIXED(218), CLAY_SIZING_GROW(0)}, .layoutDirection = CLAY_TOP_TO_BOTTOM, .childGap = 2, .padding = CLAY_PADDING_ALL(10)},
        ...
        for (uint32_t j = 0; j < JOINT_COUNT; ++j) {
            ...
            const int depth = p == NT_SKELETAL_NO_PARENT ? 0 : (p < 3 ? 1 : (p < 5 ? 2 : 3));
...
static void declare_properties(void) {
    CLAY({.layout = {.sizing = {CLAY_SIZING_FIXED(290), CLAY_SIZING_GROW(0)}, .layoutDirection = CLAY_TOP_TO_BOTTOM, .childGap = 8, .padding = CLAY_PADDING_ALL(12)},
        ...
        for (int axis = 0; axis < 3; ++axis) {
            char label[8];
            (void)snprintf(label, sizeof label, "%s", axes[axis]);
...
static void declare_ui(float fb_w, float fb_h) {
    ...
    CLAY({.layout = {.sizing = {CLAY_SIZING_GROW(0), CLAY_SIZING_GROW(0)}, .layoutDirection = CLAY_TOP_TO_BOTTOM, .childGap = 10, .padding = CLAY_PADDING_ALL(10)}}) {
        ...
        CLAY({.layout = {.sizing = {CLAY_SIZING_GROW(0), CLAY_SIZING_GROW(0)}, .layoutDirection = CLAY_LEFT_TO_RIGHT, .childGap = 10}}) {
            declare_joint_list();
            CLAY({.id = CLAY_ID(STAGE_ID), .layout = {.sizing = {CLAY_SIZING_GROW(0), CLAY_SIZING_GROW(0)}, .padding = CLAY_PADDING_ALL(12)}}) {
...
static void draw_stage(void) {
    ...
    mat4 vp;
    float eye[3];
    make_camera_vp(vp, stage_w / stage_h, eye);
    ...
    for (uint32_t j = 0; j < JOINT_COUNT; ++j) {
        ...
        const float *color = j == (uint32_t)s_selected_joint ? (const float[4]){1.0F, 0.9F, 0.2F, 1.0F}
                                                             : (in_selected_subtree(j) ? (const float[4]){1.0F, 0.55F, 0.15F, 1.0F} : (const float[4]){0.30F, 0.85F, 0.95F, 1.0F});
...
static void ensure_ids(void) {
    ...
    for (uint32_t j = 0; j < JOINT_COUNT; ++j) {
        ...
        s_joint_ids[j] = nt_hash32_str(name).value;
...
static void frame(void) {
    ...
    if (ready) {
        ...
        memcpy(uniforms.view, stage_vp, sizeof stage_vp);
        memcpy(uniforms.proj, stage_vp, sizeof stage_vp);

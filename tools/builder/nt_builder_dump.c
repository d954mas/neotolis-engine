...
static void print_atlas_details(const uint8_t *asset_data, uint32_t asset_size) {
    ...
    if (ahdr->magic != NT_ATLAS_MAGIC || ahdr->version != NT_ATLAS_VERSION) {
        return;
    }
    ...
    const uint8_t *ptr = asset_data + sizeof(*ahdr) + page_bytes;
    ...
    for (uint32_t r = 0; r < count; r++) {
        ...
        NT_LOG_INFO("    [%u] page:%u %ux%u verts:%u flags:0x%02x origin:(%.2f,%.2f)%s%s", r, reg->page_index, reg->source_w, reg->source_h, reg->vertex_count, reg->flags, (double)reg->origin_x,
                    (double)reg->origin_y, reg->transform ? " XFORM" : "", s9_buf);
    }
}
...
static void print_clip_details(const uint8_t *asset_data, uint32_t asset_size) {
    ...
    NT_LOG_INFO("    NANM v%u joints:%u rig:0x%016llX bytes:%u (expect %u)", header.version, header.joint_count, (unsigned long long)header.rig_compat_id, asset_size, (unsigned)nt_anm_size(&header));
    NT_LOG_INFO("    duration:%.3fs samples:%u sampled rows t:%u q:%u s:%u", (double)header.duration, header.sample_count, header.n_t, header.n_q, header.n_s);

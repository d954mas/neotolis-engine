    ...
    for (uint16_t ci = 0; ci < cc; ci++) {
        ...
        for (uint16_t s = 0; s < sc; s++) {
            ...
            if (is_q) {
                ...
                if (total < max_c)
                    curves[total++] = (nt_curve_t){f0x, f0y, (float)(px + d1x), (float)(py + d1y), (float)(px + d2x), (float)(py + d2y)};
                px += d2x;
                py += d2y;
            } else {
                int16_t d2x = read_varlen_delta(&rp), d2y = read_varlen_delta(&rp);
                ...
                if (total < max_c)
                    curves[total++] = (nt_curve_t){f0x, f0y, (f0x + f2x) * 0.5f, (f0y + f2y) * 0.5f, f2x, f2y};
                px += d2x;
                py += d2y;
            }
        }
...
static uint32_t encode_glyph_v3(const stbtt_vertex *v, int nv, uint8_t *buf) {
    ...
    for (uint16_t ci = 0; ci < cc; ci++) {
        uint16_t sc = 0;
        while (vi < nv) {
            if (v[vi].type != STBTT_vmove) {
                vi++;
                continue;
            }
            for (int p = vi + 1; p < nv && v[p].type != STBTT_vmove; p++)
                if (v[p].type == STBTT_vline || v[p].type == STBTT_vcurve)
                    sc++;
            if (sc > 0)
                break;
            vi++;
        ...
        for (uint16_t s = 0; s < sc; s++) {
            if (v[vi].type == STBTT_vcurve) {
                ...
                write_vl(&wp, v[vi].cx - px);
                write_vl(&wp, v[vi].cy - py);
                write_vl(&wp, v[vi].x - px);
                write_vl(&wp, v[vi].y - py);
            } else {
                write_vl(&wp, v[vi].x - px);
                write_vl(&wp, v[vi].y - py);
            }
...
#define NGLYPH 1000
    ...
    for (int g = 0; g < found; g++) {
        stbtt_FreeShape(&font, vert_cache[g]);
        free(v3_data[g]);
    }
    free(fdata);
    return 0;

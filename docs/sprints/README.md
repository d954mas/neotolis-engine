...
Этот каталог переводит `docs/spec/` (ранее `docs/neotolis_engine_spec_1.md`) в пошаговый delivery-план. Репозиторий пока пустой, поэтому план построен как greenfield-реализация без предположений о существующем коде.

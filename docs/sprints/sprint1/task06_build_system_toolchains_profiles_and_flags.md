...
Эта задача не «исправляет» спецификацию и не подменяет её. Она закрывает участок, который baseline-спека оставляет открытым: конкретную build orchestration policy. Если позже в `docs/spec/` появятся жёсткие требования к build system или флагам, roadmap должен быть синхронизирован с ними явно.
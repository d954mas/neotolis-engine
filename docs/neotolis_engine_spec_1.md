...
| `entity.list` | `{offset?, limit?, only_drawable?}` | `{total,entities:[{id,index,generation,enabled,position,drawable}]}` | **READ** live entities with compact fields (no world matrix); fully paginated against the honest `total` (two heap-free passes — the whole live range is pageable); optional `only_drawable` filter |

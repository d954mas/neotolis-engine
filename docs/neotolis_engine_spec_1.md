...
- **Plain-asset limitation (no re-download yet — #260):** for a copy-out plain asset (texture/mesh) the GPU `runtime_handle` is self-contained after activation, so rendering continues with `blob == NULL`. But `schedule_pack_redownload_if_needed` only fires for `AUX_BACKED`/`PIN_BLOB`, so an `AUTO`-evicted **plain** blob is never re-fetched — if the source is ever needed again (e.g. a WebGL context-loss re-upload) it is unrecoverable. Until #260 wires re-download + context-restore re-activation for plain assets, use `NT_BLOB_KEEP` for plain assets that must survive context loss. With explicit pack-level lifetime, `AUTO` is largely redundant for plain assets anyway (unmount already bounds the blob).

**Unmount override (D-08).** Explicit `nt_resource_unmount` overrides the pin: it proceeds (developer intent wins), emits a one-shot error log if `blob_pins > 0`, and preserves the deactivate-before-free ordering. Teardown zeroes `blob_pins` as a single source of truth, so the resolve-pass guarded decrement then finds 0 and skips — no double-free. The zero-copy consumer loses its provider and degrades to its fallback (a font renders tofu, then clears metrics once no provider remains). Invariant: **every blob-freeing path is reconciled with the pin — eviction respects it (skip), unmount overrides it (proceed + log).**
...
# 31. Radial widgets & the custom-attr image path
...
## 31.1 Route A (Clay CUSTOM) vs Route B (IMAGE + material)
...
## 31.2 Name-bound injection vocabulary
...
## 31.3 geom_mode: REGION vs GEOMETRY
...
## 31.4 The four walls (what this path does NOT do)

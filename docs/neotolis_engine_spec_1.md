...
| `entity.list` | `{offset?, limit?, component?}` | `{total,entities:[{id,index,generation,enabled,<component>:{...}}]}` | **READ** live entities: core fields (`id`/`index`/`generation`/`enabled`) plus each present component serialized as a **named group** (e.g. `transform`, `drawable`) by the generic `nt_entity_introspect` walk — the obs layer names no component, so a new component appears here with zero edits; no world matrix; fully paginated against the honest `total` (two heap-free passes); optional `component` filter by name (unknown component → `bad_params`) |

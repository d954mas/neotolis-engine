...
**Status:** architecture specification v0.5 (2026-09-19), implementation in progress under epic #472; the pose ABI, FK, binding math, rig identity, clip sampling over a base pose and sampled rows, the track clock (driven by the skeletal showcase's Skinned Meshes scene), the NSKL/NSKN/NANM wire formats at version 5 (NANM = base pose + sampled rows, no bake certificate) including both sets of bounds, their builder encoders, the runtime adapters, the glTF import of a rig, its skin binding, its skinned meshes and clips, GPU palette staging, `skin_comp`, the skinned mesh renderer and its shader ABI, and the `mix`/`override` composition kernels are implemented; runtime banks are not, and additive composition comes after v1 (#552). Function names of unimplemented parts are provisional; responsibilities, coordinate spaces, ownership, memory and behavior are normative. Changes to this chapter land together with the code that implements them.

The [skeletal showcase](../../../examples/skeletal_showcase/README.md) composes these modules.
...
- **CPU:** clips → sampled track poses → mix/override (additive after v1) → procedural edits/IK → FK → skinning palette → `skeletal_gpu` staging → GPU (frame1 = frame0, alpha 0).
...
Immutable, independently loadable resource: duration, `rig_compat_id`, version, a **base pose** (the rig's local rest with every still channel written in) and **sampled rows** — the joint components that move, on one uniform grid (§7.2). Every clip is absolute; the additive reference identity arrives in the header with #552, after v1. The builder-computed bounds `r_joints`, `r_root`, `s_max` (§14) travel in the NANM header; the glTF clip import measures them, and a hand-built clip supplies its own (0 is a measurement — nothing moves — not a sentinel). There is no bake certificate: the bank bakes the clip's own grid (§10). Skeleton owns no clips and declares no closed clip list. New clips are added by mounting new packs and resolving them by name (R6).
...
Per-joint factors are a plain `const float *` of `joint_count` entries, or NULL for 1 everywhere. The length is the pose's `joint_count` by contract, so no view type carries a second count. `mix` reads them as **weights** — per-joint ratios ≥ 0 without upper bound (R3: 3 = three times the influence of a gain-1 input); `override` (and the post-v1 `additive`, #552) as a **mask** — per-joint fractions in [0,1]. The parameter name and the kernel's documented range are the contract. Both ranges are plain comparisons asserted independently of `NT_SKELETAL_CHECKS` (they also reject NaN), as are finite gains ≥ 0; finite weights are a numerical check under the flag.
...
Only one new component: **`skin_comp`**, whose sole SoA field is a by-value `nt_deformation_binding_t`. Mesh/material/transform/color stay in `mesh_comp`, `material_comp`, `transform_comp`, `drawable_comp`.
...
**Contract.** `nt_skeletal_sample(clip, time, out)` writes `joint_count` local transforms: `out` is a copy of `base` with every row interpolated on top. `time` is a `double` in `[0, duration]`, asserted. For rows `f = time·inv_step` in double, `i = floor(f)` clamped so `i+1 ≤ sample_count−1`, `u = (float)(f − i)`; `u == 0` copies block `i` and `u == 1` copies block `i+1`, both bit for bit, so a grid time (within `2⁻²⁰` of an interval end) reproduces its stored sample bit for bit. `u` within `2⁻²⁰` of 0 or 1 is snapped to it: `time·inv_step` lands a ulp off its integer for most grid times of a non-binary duration, and the snap keeps those exact copies instead of lerps (`2⁻²⁰` of one grid interval is far below any authored key spacing); the clamp at `time == duration` yields `u == 1` the same way. Otherwise T/S lerp as `a·(1−u) + b·u` and Q takes the shortest-path normalized lerp (`d = dot(a,b)`, `b' = −b` when the sign bit of `d` is set, else `b`, `q = normalize(a·(1−u) + b'·u)`; the sign bit keeps it branch-free, and `d = −0` is a 180° tie either way). Random seek and reverse need no cursor because the index is computed, not stepped. The clip carries no sampler mode: stepped or nearest-frame playback is a player policy, the game quantizes `time` before the call. The glTF importer evaluates every channel — LINEAR, CUBICSPLINE and STEP alike — onto the grid at a developer-chosen rate and reports the runtime-vs-source error (§16); the runtime never sees a cubic or a key. Direct-access codec (§16). `out` is a caller-owned buffer of `joint_count` entries and must not overlap `base`.

### 7.3 Composition kernels

Stateless kernels over local poses (`nt_skeletal.h`); the game chains them in any order. `mix` and `override` are implemented; `additive` comes after v1. Preconditions per §2 item 5.

**`nt_skeletal_mix(inputs, input_count, defaults, joint_count, out)`** — normalized N-way mixing. Input `t` (`nt_skeletal_mix_input_t`) has a pose view, optional joint weights `bw_t` (≥ 0) and a gain `g_t` that is 0 or in `[2⁻⁶⁰, 2⁶⁰]`. `out` overlaps neither `defaults` nor any input pose; unlike `override`, mix has no in-place form, as in ozz and Unreal: its callers blend track samples that live in their own scratch, and the separate output keeps `out` `restrict`.

Per joint: `w_t = g_t·bw_t[j]`; `W = Σ w_t`; if `W == 0` copy `defaults[j]`; else T/S = `Σ (w_t/W)·x_t`. Quaternion: a running sum `A`, starting at zero, over the inputs in supplied order — for each `q` with `w > 0`, flip its sign so `dot(A,q) ≥ 0`, and if `dot == 0` exactly (always true for the first contributor) use `canonicalize(q)`; `A += w·q`; `Q = normalize(A)` once (`|A + w·q|² ≥ |A|²`, so A never cancels). `canonicalize` makes `w` positive and, for `w == 0` exactly, the largest-magnitude component positive with the tie order x, y, z, w (the rig-identity rule).

Why these choices: any fixed canonicalization makes `q` and `−q` equivalent for every input including the first, and every rule has a boundary where the output's sign flips, which no consumer sees (FK, shortest-path nlerp and a further mix are sign-agnostic); the `w` rule is a compare instead of a search on the per-joint path. Never align to rest (`+170°/−170°` would average to 0° instead of 180°); never reselect a dominant reference (sign flips across gain changes). An input with `w_t == 0` at a joint reads nothing from its pose there, so a gain-0 track keeps its clock without being sampled.

**No epsilon threshold**: a sole contributor at gain 0.0001 contributes fully; to fade toward rest, add rest as an explicit input or use `override`. A fade ends by setting the gain to 0: the asserted gain range rejects one that decays toward a subnormal, where `w·x` would quantize and `1/W` overflow. A non-zero influence `w_t` is a normal float and the T/S sums stay inside float range; a sum whose `|A|²` leaves float range (small gains or joint weights) is rescaled by `1/W` before normalizing. Cost `O(J·T)`, one quaternion normalization and one `1/W` per joint. The result is an approximate rotation average, order-sensitive for widely separated rotations and not associative: **supplied input order is part of the semantics** — no implementation (SIMD, #492) may reassociate or tree-reduce across inputs; parallelism is across joints or characters only.
...
**`nt_skeletal_override(base, top, mask, alpha, joint_count, out)`** — `a = alpha·mask[j]`, both in [0,1]; T/S lerp, Q shortest-path nlerp; `a == 0` copies `base` and `a == 1` copies `top` bit for bit, so a joint outside the mask keeps `base` exactly. Provides independent strength: "80 % upper-body aim regardless of locomotion's internal gains" — not expressible through `mix` (needs per-joint compensation, and `normalize((1−a)·normalize(A) + a·q) ≠ normalize(A + w·q)`). Permits `out == base`; every other overlap is a precondition violation.

**`nt_skeletal_additive(base, delta, mask, alpha, out)`** (after v1, #552) — prepared deltas against an explicit reference: `T += a·ΔT`, `Q = normalize(Q·nlerp(identity, ΔQ, a))`, `S *= lerp(1, ΔS, a)`; missing prepared channels are neutral; `−identity` deltas take shortest-path nlerp. The builder reconstructs source channels and computes deltas against an explicit reference; the reference pose identity in the NANM header, the delta preparation and its consumers are #552 scope.

### 7.4 Transitions and interruption (recipe over §7.3)

...
One view type `nt_joint_factors_t { const float *v; uint16_t count; }`. `mix` takes it as **weights** — per-joint ratios ≥ 0 without upper bound (R3: 3 = three times the influence of a gain-1 input); `override`/`additive` take it as a **mask** — per-joint fractions in [0,1] (Asserted under `NT_SKELETAL_CHECKS`). Absent = 1 everywhere. The parameter name and the kernel's documented range are the contract; there is no second type.

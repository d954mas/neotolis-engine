...
- `nt_skeletal_gpu_flush()` uploads `[0, cursor)` through `nt_gfx_update_texture`: the complete rows as one rectangle plus the partial last row as one `h = 1` fragment (at most two calls; none when nothing was reserved). Once per frame between the last palette write and the first pass; a second call re-uploads the same bytes.

...
**No epsilon threshold**: a sole contributor at gain 0.0001 contributes fully; to fade toward rest, add rest as an explicit input or use `override`. The result depends only on gain ratios, so when a gain leaves `[2⁻⁶⁰, 2⁶⁰]` — a fade that never reaches 0 sticks at a subnormal, where `w·x` would quantize and `1/W` overflow — the kernel scales every gain by one power of two that puts the largest near 2⁵⁰, which is exact; a sum whose `|A|²` still leaves float range (tiny joint weights) is rescaled by `1/W` before normalizing. Gains are finite; joint weights times the largest gain and the T/S sums must stay inside float range. Cost `O(J·T)`, one quaternion normalization and one `1/W` per joint. The result is an approximate rotation average, order-sensitive for widely separated rotations and not associative: **supplied input order is part of the semantics** — no implementation (SIMD, #492) may reassociate or tree-reduce across inputs; parallelism is across joints or characters only.

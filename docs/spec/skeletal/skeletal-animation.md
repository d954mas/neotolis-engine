...
**`nt_skeletal_mix(inputs, input_count, defaults, joint_count, out)`** — normalized N-way mixing. Input `t` (`nt_skeletal_mix_input_t`) has a pose view, optional joint weights `bw_t` (≥ 0) and a gain `g_t ≥ 0`; `out` overlaps neither `defaults` nor any input pose. Per joint: `w_t = g_t·bw_t[j]`; `W = Σ w_t`; if `W == 0` copy `defaults[j]`; else T/S = `Σ (w_t/W)·x_t`. Quaternion: running accumulator in supplied input order — seed `A = w_first·canonicalize(q_first)` where `canonicalize` makes `w` positive and, for `w == 0` exactly, the largest-magnitude component positive with a fixed tie order (x, y, z, w: the rig-identity rule). Any fixed rule gives the `q/−q` equivalence and every rule has a boundary where the output's sign flips, which no consumer sees (FK, shortest-path nlerp and a further mix are sign-agnostic); the `w` rule is a compare instead of a search on the per-joint path; for each next `q`, flip its sign so `dot(A,q) ≥ 0`, and if `dot == 0` exactly use `canonicalize(q)`; `A += w·q`; `Q = normalize(A)` once (`|A + w·q|² ≥ |A|²`, so A never cancels; a total so tiny or huge that `|A|²` leaves float range is rescaled by `1/W` first, which lands in `[1/√T, 1]`). Equivalently, A starts at zero and the first contributor's exact zero dot takes the canonical branch. An input with `w_t == 0` at a joint reads nothing from its pose there, so a gain-0 track keeps its clock without being sampled. Seed canonicalization makes `q` and `−q` equivalent for every input including the first. Never align to rest (`+170°/−170°` would average to 0° instead of 180°); never reselect a dominant reference (sign flips across gain changes). **No epsilon threshold**: a sole contributor at gain 0.0001 contributes fully; to fade toward rest, add rest as an explicit input or use `override`. Cost `O(J·T)`, one quaternion normalization and one `1/W` per joint. The result is an approximate rotation average, order-sensitive for widely separated rotations and not associative: **supplied input order is part of the semantics** — no implementation (SIMD, #492) may reassociate or tree-reduce across inputs; parallelism is across joints or characters only.

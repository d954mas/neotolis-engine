...
- **Base pose** `base[J]` (`nt_skeletal_trs_t`): the rig's local rest with every *still* channel written in — a channel whose grid samples are all equal (for a rotation, the negation of the first sample counts too: `q` and `−q` are one rotation) — while every channel the source does not animate keeps the rest. A sample starts as a copy of it, so the clip needs no defaults argument and a joint the clip never mentions poses exactly like the rig.
...
The rig importer computes `rig_compat_id` by calling
`nt_skeletal_rig_compat_id` rather than reimplementing the schema and stamps
the rig, its clips and its bindings with that one value; the encoders write the
field as given, so what ships is what the producer stamped. The pack CRC32 detects accidental corruption, does not

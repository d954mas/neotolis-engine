...
**Status:** architecture specification v0.4 (2026-09-18), implementation in progress under epic #472; the pose ABI, FK, binding math, rig identity, clip sampling with the object curve, the track clock, the NSKL/NSKN/NANM wire formats including both sets of bounds, their builder encoders, the runtime adapters and the glTF import of a rig, its skin binding, its skinned meshes and its clips (with the error report) are implemented; mix/override/additive, banks, GPU staging, the renderer, the object curve export and the §14 culling helper are not. Function names of unimplemented parts are provisional; responsibilities, coordinate spaces, ownership, memory and behavior are normative. Changes to this chapter land together with the code that implements them.
...
**A `matrix` node on a rig path is decomposed** in the builder's rig importer — never in `engine/skeletal`, and never during parse, because a sheared matrix in a static-mesh scene must not abort a mesh-only build. Column lengths and the rotation run in double: a non-finite element or a bottom row other than `(0, 0, 0, 1)` is rejected before any column is measured, a column shorter than `1e-6` cannot recover a rotation and is rejected, a negative determinant becomes a negative X scale, and the quaternion comes from the standard trace/largest-diagonal extraction canonicalized to `w ≥ 0` (and `-0` to `+0`) so the stored bits do not depend on which branch ran. The decomposition is accepted only if `nt_skeletal_mat34_from_trs` recomposes every linear element within `64·FLT_EPSILON·|scale[col]|` of its column and the translation exactly; anything else is shear the runtime cannot represent, and shear is a logged diagnostic followed by `NT_BUILD_ASSERT`. The decomposed bits are the rest bits `rig_compat_id` hashes.

### 3.2 Clip (`NANM`)

Immutable, independently loadable resource: duration, `rig_compat_id`, the additive reference identity (`additive_ref_id`, 0 for an absolute clip), version, joint tracks, interpolation rules and an optional **object curve** (one TRS signal for the whole character, §7.5). The builder-computed bounds `r_joints`, `r_root`, `s_max` (§14) travel in the NANM header; the glTF clip import measures them, and a hand-built clip supplies its own (0 is a measurement — nothing moves — not a sentinel). There is no bake certificate: the bank bakes the clip's own grid (§10). Skeleton owns no clips and declares no closed clip list. New clips are added by mounting new packs and resolving them by name (R6).
...
```c
nt_skeletal_bank_init(&bank, &desc);   /* CPU only: skeleton, binding, clips[], format, width → one allocation */
...
**`init`** (CPU only, no gfx call) takes `desc.width` (0 = `min(2048, gpu_caps.max_texture_size)`) and computes the layout: `fpr = floor(width / 3P)` frames per texture row; a duration-zero clip stores exactly one frame at time 0, without extra endpoint or STEP frames, in either playback mode. For positive durations: `F = sample_count` frames at the clip's own grid times `i·duration/(sample_count−1)` — the bank has no rate of its own; the developer chose the clip's rate at import (§16) and read its error report — plus two frames per distinct STEP timestamp `t_s` (sampled at `t_s⁻` and `t_s`) with a small per-clip segment table; a clip with `sample_count == 1` stores one frame at 0 plus its STEP pairs; `height = ceil(frames_total / fpr)`. Asserts: `3P ≤ width` (P ≤ 682 at the WebGL2 minimum 2048), `height ≤ max_texture_size`, origins addressable in `uint16`, every clip absolute (`additive_ref_id == 0`) with equal `rig_compat_id`. GPU storage is `width·height·bytes_per_texel`; the CPU copy is the same size; descriptors and bake workspace are counted separately. Baked-only characters allocate no PoseInstance. Adding clips = a replacement bank (init + bake with the expanded list), swapped in after outstanding draws finish; no append API, no implicit growth.
...
Single-clip playback uses `r = r_joints + reach·s_max`. `r_joints` is the largest joint-origin distance from the skeleton origin and `s_max` the largest product of local `max(abs(s))` along an ancestor chain (a conservative stretch bound, the largest singular value of every model matrix's linear part, including hierarchical shear; individual local scales or model-matrix column lengths do not bound it), both **measured** over the importer's dense set — every grid time, every authored key and three sub-samples per grid interval, on the decoded runtime pose — and stored rounded towards +∞. `r_root` is exact (the norm of a lerp is convex). The two others are maxima over that set, not a proof over the interpolated interior: between sub-samples an interpolated rotation can carry a joint a fraction of a percent farther (≈ `1 − cos(Δθ/8)` of the offset per interval, 0.2 % at 30° per frame). A tight measured value serves culling better than a chain-sum bound (bone lengths added up against a folded pose); the culling helper adds its own margin, as the manual bounds margins of Unity's SkinnedMeshRenderer and Unreal's Bounds Scale do.
...
is a plain `uint16_t` (v1 of this layout = 4) compared **exactly** in all three
...
| 8 | `u64 rig_compat_id` | |
| 16 | `f32 reach` | joint space (§3.4), finite and ≥ 0; validated by the builder, rejected by the activator when negative or non-finite |
| 20 | `f32 any_pose_radius` | skeleton space (§14), finite and ≥ 0; validated by the builder, rejected by the activator when negative or non-finite |
...
| 55 | `u8 _pad` | zero |
| 56 | `f32 r_joints` | finite, ≥ 0; §14 |
| 60 | `f32 r_root` | finite, ≥ 0; §14 |
| 64 | `f32 s_max` | finite, ≥ 0; §14 |
...
`skin_drop_tolerance`, default `NT_BUILDER_SKIN_DROP_TOLERANCE`),
`nt_builder_add_scene_skin_binding` and `nt_builder_add_scene_clip` (with its
`sample_fps` and `nt_builder_clip_report_t`). A source violation of any
...
glTF is the normative source reference; import selects the canonical rig (skin/node), helper joints and identity explicitly so independently imported clips reproduce the same identity. The scene API publishes the flattened nodes with their parent and skin index and the animations by name; the rig and clip importers read rest transforms, skins and samplers from the parsed glTF directly.

**Implemented.** `nt_builder_import_rig` builds the rig of §3.1 out of one skin. `nt_builder_add_scene_skinned_mesh` exports one primitive: it reads every `JOINTS_n/WEIGHTS_n` pair through sparse-capable unpacking, checks each source accessor's type (`JOINTS_n` VEC4 unsigned byte or short and not normalized, `WEIGHTS_n` VEC4 float or normalized unsigned byte/short, per the glTF accessor table; a FLOAT or normalized joint lane would unpack to a fraction, not an index), keeps the four largest influences per vertex with ties broken towards the lower joint index, gates each vertex on `Σ dropped / Σ source ≤ skin_drop_tolerance` (in `[0, 1]`, default 0.02) after a logged diagnostic, logs one warning per primitive that had a vertex reduced, naming how many were and the largest dropped ratio, and renormalizes what it kept. `nt_builder_add_scene_skin_binding` writes the binding from the same rig: inverse binds from the skin's accessor (identity when it has none), the rig's palette remap, `reach` measured over the *source* influences of every primitive of every node that uses the skin — so the order in which meshes and binding are exported does not matter — and `any_pose_radius` from the rest hierarchy per §14. Rejected as content errors, each a diagnostic then `NT_BUILD_ASSERT`: a glTF that fails `cgltf_validate` (accessors outside their buffer views, mismatched attribute counts, out-of-range indices or sparse indices, a parent cycle — checked in parse, before any reader trusts the data); an unpaired or non-consecutive influence set; a vertex that weights one palette entry twice, or carries a negative, non-finite or all-zero weight set, or, on a lane with a non-zero weight, addresses an index at or above the palette count; an `inverseBindMatrices` accessor that is not MAT4 float over at least the palette, or holds a non-finite element or a bottom row other than `(0, 0, 0, 1)`; morph targets on a skinned primitive (they move the bound vertices `reach` is measured over); a rig node with a non-finite rest translation or scale, or a non-unit rest rotation; a `matrix` rig node that is non-finite, non-affine, with a column shorter than `1e-6` or not T·R·S (§3.1), or that carries translation/rotation/scale next to its matrix; a skin that lists one node twice; a joint chain deeper than 256; an unnamed or empty-named rig node (a joint id is the hash of its name); two rig nodes whose names hash to one joint id; joints spanning several scene roots; a skin joint outside the `skeleton_root` cut; a skin without joints, or a skin palette or rig above 65535 joints; a skin index the scene does not have; a skinned primitive without a VEC3 `POSITION` (binding export; the mesh export sees it only through its layout); a UINT8 `JOINTS` layout over a palette above 256 entries; a skinned vertex at a non-finite distance from its joint (`inverse_bind·POSITION`); a skinned-mesh export of a mesh no node instantiates with the rig's skin; and a binding export for a skin no node uses (its reach would be zero).

**Implemented: clip import.** `nt_builder_add_scene_clip(ctx, scene, animation_index, rig, sample_fps, resource_id, &report)` turns one glTF animation into an absolute NANM clip on the rig. Channels map to joints by `joint_id = nt_hash32_str(node name)`, the same path for the rig's own scene and for another one; every animated node's local rest TRS must be bit-identical to the rig's (a differing rest is a different rig, §3.1). The source curves are evaluated in double per the glTF sampler rules — held before the first key and after the last, LINEAR lerp for T/S and shortest-path slerp for Q (a pair closer than `1e-9` to parallel takes a normalized lerp, since exporters write identical adjacent keys and dots a few ulps above 1), STEP hold, CUBICSPLINE as the cubic Hermite with `[in, value, out]` tangents scaled by the key interval and the rotation result normalized; accessors are unpacked once through the sparse-capable path, rotation keys are normalized (glTF allows normalized BYTE/SHORT quaternions; a key with `|q|² < 0.5` is a zero key, rejected). Every animated node must also hang under the node whose name hashes to its joint's parent (the cut root excepted): the same names under another hierarchy are a different rig. **The grid step is `1/sample_fps`** (up to the float rounding of the shipped duration): the frame count is the nearest whole number of frames of the source length, `sample_count = round(source_duration·sample_fps) + 1` (at least 2), the shipped `duration` is `(sample_count−1)/sample_fps`, so a clip authored at that rate lands its keys on grid samples; a source that is not a whole number of frames at the chosen rate (beyond 1e-3 frame of exporter noise) holds its last pose for the fraction or loses it, is logged, each STEP channel logs the keys it drops past the end (keys within the same tolerance land on the end instead, as one key holding the last value), and the report carries the duration that shipped — the Unreal import rule, because the runtime grid is uniform. LINEAR and CUBICSPLINE channels are evaluated at the grid times; STEP channels keep their authored keys at or before the end; a channel whose grid samples, or whose keys, are all bit-identical folds to CONSTANT (a rotation's samples may also all be the exact negation of the first: q and −q are one rotation; the ozz/ACL rule — a cubic bulge between grid samples shows in the report, never silently); a clip with nothing sampled ships `sample_count == 1` and its exact source length; the object channels stay absent (§7.5). The encoded bytes are read back through `nt_skeletal_clip_view` and sampled by the runtime over a dense set of times — every grid time, every authored key time and three sub-samples at `(4i+k)/4` of each grid interval — against the exact curves through the same FK; that pass fills the report (`sample_count`, `duration`, `cpu_error_lin` = the largest Frobenius distance between the 3×3 parts of a joint's model matrix, unitless, `cpu_error_t` = the largest translation distance in scene units, each with the time and joint it was found at) and the §14 bounds `r_joints`, `r_root`, `s_max`, accumulated in double and stored rounded towards +∞. The builder never fails a build on interpolation error; the build script reads the report. Rejected as content errors, each a diagnostic naming the animation and, where one exists, the node, then `NT_BUILD_ASSERT`: an animation index the scene does not have, a non-finite or non-positive `sample_fps`, an animation without channels, a channel without a target node or with an unnamed one, a `weights` channel (morph targets) or an unknown target path, a target that carries a `matrix` (glTF forbids one on an animated node), a target outside the rig, a parent or rest mismatch, a curve that evaluates outside the float range, a rate whose grid overflows or has no finite step, two channels on one node property, an input accessor that is not SCALAR FLOAT or an output that is not VEC3 FLOAT (T/S) or VEC4 FLOAT/normalized 8- or 16-bit (Q), an accessor that does not unpack whole, input times that are negative, non-finite or not strictly increasing, a non-finite output, a rotation key too short to normalize. Measured on the Khronos assets at 24 fps: Fox Survey and Walk and CesiumMan land within 0.3° and 0.07 cm; Fox Run, authored at 27.8 frames with its late keys at fractional frame .8 — 0.2 frames from the nearest grid sample, off every sub-sample — reports 4.7° and 1.8 cm, the number a developer reads before choosing another rate.

## 17. Modules and composition checks

- `skeletal` (`engine/skeletal`, one module `nt_skeletal`): `nt_skeletal.h` — pose ABI, skeleton view, 3×4 kernels, FK, sockets, rig identity, skin binding view and palette build, plus sample, the object curve, `nt_skeletal_clip_view` and `tracks_advance`; mix/override/additive land with their issue. A radius bounds helper (§14) is added with its first consumer.

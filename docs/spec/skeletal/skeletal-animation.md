...
Per-joint factors are a plain `const float *` of `joint_count` entries, or NULL for 1 everywhere. The length is the pose's `joint_count` by contract, so no view type carries a second count. `mix` reads them as **weights** — per-joint ratios ≥ 0 without upper bound (R3: 3 = three times the influence of a gain-1 input); `override` (and the post-v1 `additive`, #552) as a **mask** — per-joint fractions in [0,1]. The parameter name and the kernel's documented range are the contract. Both ranges are plain comparisons asserted in every assert mode (they also reject NaN); finite weights are a numerical check under `NT_SKELETAL_CHECKS`.

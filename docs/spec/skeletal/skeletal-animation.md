...
Why: any fixed `canonicalize` makes `q` and `−q` equivalent on every input, the first included; the output's sign is invisible to FK, shortest-path nlerp and a further mix, and the `w` rule costs a compare instead of a search. Never align to rest (`+170°/−170°` would average to 0°, not 180°) or to a dominant input (it changes with gains). An input with `w_t == 0` at a joint reads nothing from its pose there, so a gain-0 track keeps its clock unsampled.

**No epsilon threshold**: a sole contributor at gain 0.0001 contributes fully; to fade toward rest, add rest as an input or use `override`. A fade ends by setting the gain to 0 — the asserted range rejects one decaying toward a subnormal, where `w·x` quantizes and `1/W` overflows. Non-zero `w_t` are normal floats and T/S sums stay in float range; a sum whose `|A|²` leaves float range is rescaled by `1/W` before normalizing. Cost `O(J·T)`, one normalization and one `1/W` per joint. **Supplied input order is part of the semantics** (the average is order-sensitive for widely separated rotations and not associative): no implementation (SIMD, #492) may reassociate or tree-reduce across inputs; parallelism is across joints or characters only.

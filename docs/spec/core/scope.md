...
NTPACK, shader/material, mesh/sprite/text rendering, audio, builder) and what is
explicitly out of scope (editor, scripting, physics, WebGPU backend). The large
`nt_ui` note documents the minimal immediate-mode UI module built on vendored Clay:
lifecycle, transforms, interaction model, widgets, scroll physics, popups, menus.

Related: [Core Principles](principles.md), [Module Layout](module-layout.md), [Rich Text](../ui/rich-text.md), [Radial Widgets](../ui/radial-widgets.md)

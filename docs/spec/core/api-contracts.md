...
The target format is the first entry of the blob codec's order that the GPU
supports and the build admits (`NT_BASISU_HAS_BC7/ASTC/ETC2`) — ETC1S:
`ETC2_RGBA8`/`ETC2_RGB8` (by the blob's alpha) → `BC7_RGBA` → `ASTC_4x4_RGBA`;
UASTC: `ASTC_4x4_RGBA` → `BC7_RGBA` → ETC2 — then `RGBA8` as the always-available
fallback ([runtime formats](../assets/runtime-formats.md#texture-activation-ttex)).
`BC7_RGBA` is eligible only when both base dimensions are multiples of 4;
otherwise selection continues with the next candidate of that order.

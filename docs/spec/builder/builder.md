...
The graceful channel is scoped to the atlas builder — it lets a batch of sprites survive one bad member. Atlas content errors are collected into a sticky, add-order-stable accumulator on the context. One bad sprite does not abort the atlas — the remaining sprites in the same atlas are still validated so bad sprites are reported together, in stable add order, and a poisoned pack writes no `.ntpack`. The accumulator holds up to `NT_BUILD_MAX_ERRORS` (256) content errors; beyond that the tail is dropped and `nt_builder_errors_truncated()` returns true (the retained prefix stays add-order-stable). A single-dimension-oversized image file (a PNG whose width or height exceeds the decoder's `STBI_MAX_DIMENSIONS`) is reported as `IMAGE_TOO_LARGE`, not `CORRUPT_IMAGE` — the atlas builder disambiguates the decode reject via the decoder's failure reason. Callers read the list after `finish_pack`:
...
The "report bad sprites together" guarantee is **per-atlas**. Once a pack is poisoned, `begin_atlas` stops packing for every subsequent atlas — a multi-atlas pack therefore surfaces only the first failing atlas's errors, and the frontend re-runs the build after fixing them to see the next atlas's diagnostics. This between-atlas hard stop is intended: it avoids validating downstream atlases whose inputs may depend on the poisoned one, and keeps the error list bounded to one atlas's worth of actionable content.

A poisoned `begin_atlas` is **not** a pure no-op: it opens a *logical skipped atlas* that still requires a matching `end_atlas`, so the caller's begin/add/end lifecycle stays balanced. The caller-visible contract is unchanged from the packing path — a nested `begin_atlas`, an `atlas_add*`/`end_atlas` with no atlas open (real or skipped), and a `finish_pack` with a skipped atlas still open all assert. Programmer-contract validation also still fires on the skipped path. `begin_atlas` runs a single full-domain validator (`atlas_assert_opts`) before the poison gate, covering the whole `nt_atlas_opts_t` domain: out-of-range `format`/`shape`/sampler `filter_*`/`wrap_*` enums, `max_vertices` outside `3..16`, a non-positive/non-finite `pixels_per_unit`, Basis `compress` on a non-RGBA8/RGB8 `format`, and a mipmap `filter_min` with `gen_mipmaps = false` on a RAW page. The pure per-sprite opts (shape/rotate/max_vertices), missing/unreadable sprite files, bad glob patterns, and the atlas-shape-dependent sprite cross-field rules (slice9 requires RECT; per-sprite `extrude > 0` requires an effective RECT shape) also fire on the skipped path — a poisoned pack never swallows a caller bug. Only content-dependent sprite work is skipped.
...
**Storage:** one `atlas_<key>.bin` file per cache hit, containing the placement table and the composed page pixels. On hit, the pipeline skips pack/compose/cache_write; `debug_png`, serialize and register still run — the page pixels `debug_png` needs come from the cache read just as they would from a fresh compose.

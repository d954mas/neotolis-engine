...
nt_builder_add_scene_clip           /* NANM: the animation of that name on the rig at sample_fps, fills nt_builder_clip_report_t */
...
**glTF skin content** is validated explicitly, because `cgltf_validate` checks structure only (buffer bounds, attribute counts, indices, hierarchy), never weight or matrix values. `nt_builder_parse_glb_scene` rejects a glTF that fails `cgltf_validate` (accessors outside their buffer views, mismatched attribute counts, out-of-range indices or sparse indices, a parent cycle) before any reader trusts the data. The rig import rejects an unnamed or empty-named rig node, two rig nodes whose names hash to one joint id, a non-finite rest translation or scale or a non-unit rest rotation, joints spanning several scene roots, a skin that lists one node twice, a skin without joints, a skin index the scene does not have, a skin palette or rig above 65535 joints, a joint chain deeper than 256, a node with both a `matrix` and TRS, and a `matrix` node that is non-finite, non-affine, with a column shorter than `1e-6` or whose decomposition does not recompose (skeletal §3.1). The skinned-mesh and binding exports reject unpaired or non-consecutive influence sets, source accessor types outside the glTF list, a vertex weighting one palette entry twice or carrying a negative, non-finite or all-zero weight set, an index at or above the palette count on a lane with a non-zero weight, a vertex losing more than `skin_drop_tolerance` to the top-four reduction, morph targets on a skinned primitive, a skin whose primitive has no VEC3 `POSITION` (binding export), a vertex at a non-finite distance from its joint through its inverse bind, a mesh no node instantiates with the rig's skin, a skin no node uses (the binding would carry a zero reach), and an `inverseBindMatrices` accessor that is not MAT4 float over at least the palette, holds a non-finite element or has a bottom row other than `(0, 0, 0, 1)`. The clip import rejects an animation name matching no animation of the scene or several (`NULL` and `""` are one name, the unnamed animation; the diagnostic lists the scene's animations), a non-finite or non-positive `sample_fps`, an animation without channels, a channel without a target node or with an unnamed one, a channel that animates neither translation, rotation nor scale (morph `weights`, or a path cgltf does not know), a target that carries a `matrix`, a target outside the rig, an animated node whose parent is not the node its joint's parent is named after (root joints excepted; the clip file's rest is never compared), two channels on one node property, a curve that evaluates outside the float range, a rate whose grid overflows or has no finite step, an input accessor other than non-normalized SCALAR FLOAT with at least one key, an output accessor outside the glTF list (VEC3 FLOAT; for a rotation VEC4 FLOAT or normalized BYTE/SHORT), accessors that do not unpack whole, input times that are negative, non-finite or not strictly increasing, a non-finite output and a rotation key too short to normalize (Skeletal Animation, "Builder, codec, wire formats"). Each is a logged diagnostic followed by `NT_BUILD_ASSERT`, per the skeletal chapter's builder policy.

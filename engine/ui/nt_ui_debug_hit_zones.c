...
nt_ui_overlay_pen_t nt_ui_internal_overlay_bind(const nt_ui_context_t *ctx, nt_material_t smat) {
    ...
    const bool base = smat.id == ctx->sprite_material.id && ctx->base_custom_bytes != 0U;
    return (nt_ui_overlay_pen_t){.atlas = ctx->atlas, .region = ctx->white_region, .block = base ? ctx->base_custom_attrs : NULL, .block_bytes = base ? ctx->base_custom_bytes : 0U};
}
...
void nt_ui_internal_emit_filled_quad_m(const nt_ui_overlay_pen_t *pen, const float v[4][2], const float model[16], uint32_t color) {
    ...
    if (pen->block_bytes != 0U) {
        nt_sprite_renderer_set_custom_attrs(pen->block, pen->block_bytes);
    }
    nt_sprite_renderer_emit_geometry(pen->atlas, pen->region, v, 4U, indices, 6U, model, color);
}
...
void nt_ui_internal_emit_outline_m(const nt_ui_overlay_pen_t *pen, const float c[4][2], float thickness, const float model[16], uint32_t color) {
    ...
    for (uint32_t i = 0; i < 4U; ++i) {
        ...
        nt_ui_internal_emit_filled_quad_m(pen, quad, model, color);
    }
}

/* Vertices already in world space (2D-ctx pre-projected path). */
void nt_ui_internal_emit_filled_quad(const nt_ui_overlay_pen_t *pen, const float v[4][2], uint32_t color) { nt_ui_internal_emit_filled_quad_m(pen, v, NT_MATH_MAT4_IDENTITY, color); }

void nt_ui_internal_emit_outline(const nt_ui_overlay_pen_t *pen, const float c[4][2], float thickness, uint32_t color) {
    nt_ui_internal_emit_outline_m(pen, c, thickness, NT_MATH_MAT4_IDENTITY, color);
}
// #endregion
...
void nt_ui_debug_draw_hit_zones(nt_ui_context_t *ctx, const nt_ui_target_t *target, nt_ui_debug_hit_mode_t mode, nt_font_t font, float label_size) {
    ...
    for (uint32_t i = 0; i < ctx->debug_zone_count; ++i) {
        ...
        if (is_3d) {
            nt_ui_internal_emit_filled_quad_m(&pen, pad_corners, z->m, fill);
            /* Outline visual bbox so padding is visually distinct. */
            nt_ui_internal_emit_outline_m(&pen, vis_corners, 2.0F, z->m, DEBUG_OUTLINE_COLOR);
            if (can_label) {
        ...
        nt_ui_internal_emit_filled_quad(&pen, pad_corners, fill);

        for (uint32_t k = 0; k < 4U; ++k) {
            nt_ui_internal_project_layout_to_world(z->m, vy, vh, vis_corners[k][0], vis_corners[k][1], &vis_corners[k][0], &vis_corners[k][1]);
        }
        nt_ui_internal_emit_outline(&pen, vis_corners, 2.0F, DEBUG_OUTLINE_COLOR);

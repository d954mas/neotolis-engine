...
void nt_ui_group_begin(nt_ui_context_t *ctx, const nt_ui_element_data_t *data, const Clay_ElementDeclaration *decl) {
    ...
    /* Group has no image/bg; only id + userData are engine-owned. */
    if (decl != NULL) {
    ...
    /* CUSTOM anchor with type=NONE — walker skips the handler but still uses
     * the bbox for deferred center resolution. */

...
/* Shared fill-emit helper consumed by slider fill AND progress fill (D-59-24).
 * STRETCH: a slice9-aware region stretched to ratio*track (shares the slider/progress
 * emit path). CROP: a full-size region revealed by ratio via a Clay .clip scissor
 * (texture "reveals", no distortion); slice9 borders are IGNORED in CROP — geometrically
 * incompatible with a partial reveal (D-59-23). */
 ...
 * (no-art rule, D-59-16 parity). track_w/h are the fill container size (finite > 0).

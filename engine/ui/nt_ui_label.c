...
void nt_ui_label(nt_ui_context_t *ctx, const nt_ui_element_data_t *data, const char *text, const nt_ui_label_style_t *style) {
    ...
    NT_ASSERT(isfinite(style->font_size) && style->font_size > 0.0F && style->font_size <= (float)UINT16_MAX && "nt_ui_label: font_size must be finite in (0, UINT16_MAX]");
...
void nt_ui_label_sized(nt_ui_context_t *ctx, const nt_ui_element_data_t *data, const char *text, const nt_ui_label_style_t *style, float font_size_override) {
    ...
    NT_ASSERT(isfinite(font_size_override) && font_size_override > 0.0F && font_size_override <= (float)UINT16_MAX && "nt_ui_label_sized: font_size_override must be finite in (0, UINT16_MAX]");

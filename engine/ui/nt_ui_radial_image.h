 ...
 * textured radial supports polygon + slice9 regions. The reveal look (mode +
 * dim_factor + tint) is NOT here — it is baked onto the MATERIAL at creation via
 * the u_reveal_mode / u_reveal_tint params (one material per look). The widget
 * style carries no reveal state, so N widgets sharing a material in one batch all
 * reveal identically. The radial-image material (attr_map a_radial @ loc 4 + the
 * u_reveal_mode / u_reveal_tint params) is supplied by the game per reveal look
 * (one material per look -> one batch per look). .id==0 invalid. */
typedef struct {
...
_Static_assert(sizeof(nt_ui_radial_image_style_t) == 36, "nt_ui_radial_image_style_t stable ABI (36 B)");
 ...
 * material for the chosen reveal look. */
...
/* Material param names the shader reads, set once on the material by the game at
 * creation (one material per reveal look). u_reveal_mode = {mode, dim_factor, 0, 0};
 * u_reveal_tint = {r, g, b, tint_strength} in 0..1. */

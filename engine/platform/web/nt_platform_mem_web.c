...
nt_platform_mem_t nt_platform_memory_usage(void) {
    ...
    /* uordblks = allocator in-use bytes. struct mallinfo's fields are int on the pinned emsdk 4.0.19
       (the size_t-fielded mallinfo2 is not declared there either); a >2GiB heap can read negative, so clamp. */

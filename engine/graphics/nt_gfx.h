 ...
 * recoverable failures -- processed context loss, a texture husk, sampler
 * recreation failure -- publish no set and no backend bind; gfx reports them and
 * skips the following draws of that set, so the caller has nothing to check. */

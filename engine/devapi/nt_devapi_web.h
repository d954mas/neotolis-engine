...
/* Web (Emscripten) transport for the devapi core: a push/pull ccall bridge instead of a socket.
   ...
   layer. The host calls only nt_devapi_web_install_shim once at startup; the KEEPALIVE wrappers
...
/* Install window.__devapi = {submit, poll, reset, tick, ready}. The host calls this once on the web path

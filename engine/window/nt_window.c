...
void nt_window_apply_sizes(float css_w, float css_h, float device_dpr) {
    ...
    g_nt_window.width = (uint32_t)css_w;
    g_nt_window.height = (uint32_t)css_h;
    g_nt_window.dpr = effective_dpr;
    g_nt_window.fb_width = (uint32_t)roundf(css_w * effective_dpr);
    g_nt_window.fb_height = (uint32_t)roundf(css_h * effective_dpr);
}
